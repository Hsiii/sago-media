...
export function html(body: string, status = 200) {
  return response(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light dark"><title>Sago Media</title><style>:root{color-scheme:light dark;font-family:ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#fff;color:#1d1d1f}@media(prefers-color-scheme:dark){:root{background:#000;color:#ededed}}*{box-sizing:border-box}body{margin:0}.auth{min-height:100vh;display:grid;place-items:center;padding:24px}.card{width:min(420px,100%);padding:32px;border:1px solid color-mix(in srgb,currentColor 16%,transparent);border-radius:16px;text-align:center}.mark{width:32px;height:32px;margin:0 auto 16px;border-radius:50%;display:grid;place-items:center;background:currentColor}.mark span{color:Canvas;font-size:12px;font-weight:600}h1{margin:0 0 8px;font-size:24px;letter-spacing:-.04em;text-wrap:balance}p{margin:0 0 20px;color:color-mix(in srgb,currentColor 64%,transparent);line-height:1.5;text-wrap:pretty}a{min-height:40px;padding:0 16px;border-radius:8px;display:inline-flex;align-items:center;background:currentColor;color:Canvas;text-decoration:none;font-weight:500}</style></head><body>${body}</body></html>`, status, { "Content-Security-Policy": "default-src 'none'; style-src 'unsafe-inline'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'", "Content-Type": "text/html; charset=utf-8" });
}

export function authPage(title: string, copy: string, action = "") {
  return `<main class="auth"><section class="card"><div class="mark"><span>SM</span></div><h1>${title}</h1><p>${copy}</p>${action}</section></main>`;
}
